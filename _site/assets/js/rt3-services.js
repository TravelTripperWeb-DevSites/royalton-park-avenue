      ...
      rt3api.getAllSpecialRates().then(function(response) {
             ...
             $rootScope.$applyAsync(function() {
              ...
              if(specialRates.locationHash){
                  ...
                  var totOffers = response.special_rates.length;
                  angular.forEach(response.special_rates, function(value, key) {
                    ...
                    if (tmpName == hashName) {
                          ...
                          var curKey = (key == totOffers) ? 0 : key ;
                          var navigationOffers = [];
                          for(var i=0 ; i< 4 ; i++){
                            navigationOffers[i] = response.special_rates[curKey++];
                            if(curKey == totOffers)
                              curKey = 0;
                          }
                          specialRates.navOffers = navigationOffers;
                          specialRates.curOfferIndex = key;
                          specialFound = true;
